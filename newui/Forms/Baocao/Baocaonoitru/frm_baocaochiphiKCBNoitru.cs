    ...
    public partial class frm_baocaochiphiKCBNoitru : Form
    {
        ...
        private void frm_BAOCAO_TONGHOP_TAI_KKB_DTUONG_THUPHI_Load(object sender, EventArgs eventArgs)
        {
            try
            {
                ...
                txtTNV.Init(THU_VIEN_CHUNG.LaydanhsachThunganvien(), new List<string>(){
                                      DmucNhanvien.Columns.IdNhanvien,DmucNhanvien.Columns.UserName, DmucNhanvien.Columns.TenNhanvien});
                m_dtKhoathucHien = THU_VIEN_CHUNG.Laydanhmuckhoa("NOI", 0);
                txtKhoanoitru.Init(m_dtKhoathucHien, new List<string>() { DmucKhoaphong.Columns.IdKhoaphong, DmucKhoaphong.Columns.MaKhoaphong, DmucKhoaphong.Columns.TenKhoaphong });
            }
            catch (Exception ex)
            {
        ...
        private void cmdInPhieuXN_Click(object sender, EventArgs e)
        {
            if (chkTachCDHA.Checked)
            {
                _dtData =
                  BAOCAO_NGOAITRU.BaocaoDoanhthuphongkham(
                    ...
                    Utility.sDbnull(cboDoituongKCB.SelectedValue, -1), Utility.sDbnull(txtTNV.MyCode, "-1"), Utility.ByteDbnull(cboLoaidichvu.SelectedValue, 2), Utility.sDbnull(txtKhoanoitru.MyCode, "-1"));
            ...
            string Condition = string.Format("Từ ngày {0} đến {1} - Đối tượng : {2} - Nhân viên :{3}", dtFromDate.Text, dtToDate.Text,
                                          ...
                                          txtTNV.MyCode!="-1"
                                              ? txtTNV.Text
                                              : "Tất cả");
            string reportcode = chkTachCDHA.Checked ? "baocao_chiphiKcbNoitru_chitiet" : "baocao_chiphiKcbNoitru_tonghop";
